...
print(f"Average availability over 365 days is {np.mean(ds["availiability_365"])}")
print(f"Standard deviation on minimum nights is {np.std(ds["minimum_nights"]:.2f)}")
...
float(ds2['price'][0].replace('$','')) # 将$替换为空格
...
get_ipython().system('jupyter nbconvert --ClearOutputPreprocessor.enabled=True     --to python --output=dtools/__init__.py     Practical-04-Objects_group1.ipynp')
